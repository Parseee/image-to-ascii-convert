...
void resizeImage (unsigned char* data, std::vector<unsigned char>& image, int& x, int& y) {
    int width, height;
    std::cout << "Enter preferable resolution (if (0, 0) - default): ";
    std::cin >> width >> height;
    if (width != 0 && height != 0) {
        stbir_resize_uint8 (data, x, y, 0, data, 64, 32, 0, 1);
        x = width; y = height;
    }
}
...
void convertToAscii (std::vector<unsigned char>& image, int limit) {
    int cnt = 0;
    for (auto& elem : image) {
        std::cout << density[26 - getIndex(static_cast<int>(elem))];
        ++cnt;
        if (cnt > limit - 1) {
            std::cout << '\n';
            cnt = 0;
...
int main() {
    std::string filename = "/Users/parse/Downloads/tits.png";

    int width, height;
    std::vector<unsigned char> image;
    bool success = loadImage(image, filename, width, height);
    if (!success) {
        std::cout << "Error loading raw image\n";
        return 1;
    }
